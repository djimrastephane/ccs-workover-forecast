...
12. **Seismicity model is a stationary Bernoulli trigger with fixed damage multipliers** — the induced-seismicity mechanism ([issue #1](https://github.com/djimrastephane/ccs-workover-forecast/issues/1)) draws one field-level event per (simulation, year) at a constant annual probability. It does not model: magnitude–frequency scaling (every event applies the same casing/cement multiplier regardless of size or epicentral distance); fault geometry (which wells actually cross the reactivated fault); coupling to injection rate or pressure (real stoplight protocols *reduce injection* after events, which would lower subsequent seismicity — here the protocol only affects detection); or damage accumulation across repeated events. The Seismic Monitoring Array is modelled per-well for consistency with other components, although a real array is field-level infrastructure. All seismic parameters are expert judgement — no quantitative fragility curves exist for CCS wellbores.
