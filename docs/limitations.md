...
11. **All wells start at Year 1 regardless of prior history** — the simulation assumes a greenfield fleet. Every well begins at the infant mortality phase (years 1–2, 1.5× bathtub multiplier) regardless of actual construction date. Converted legacy O&G producers entering the CCS fleet mid-life should start at an older point on the bathtub curve with accumulated degradation already reflected — they should not be assigned infant mortality risk. The **Legacy Well Conversion** scenario (2.5× failure multiplier, SCSSV disabled) is a coarse proxy for the worst-case converted-well population but does not model pre-existing age, accumulated corrosion/cement carbonation, or mixed fleets where some wells are new and others are converted. A `start_age` parameter per well class is needed to represent this correctly. See [issue #5](https://github.com/djimrastephane/ccs-workover-forecast/issues/5).
