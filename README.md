...
| Fiber Optics | Monitoring | 12 yr | 26 yr | Full workover | 85% | default assumes conventional permanent installation (strapped to tubing); **FIPA / pumped fiber exception**: dual-conduit turnaround loop (offered by providers such as Schlumberger/SLB and Optasense) — fiber sensing element pumped out and replaced at surface with no rig; override `intervention_type` to `rigless_intervention` for FIPA wells; FIPA conduit loop failure itself is still a full workover |
...
| FIPA / pumped fiber — vendor technical specifications (SLB, Optasense, Baker Hughes) | Two permanent DTS/DAS deployment architectures: (1) conventional strapped installation — fiber inside a small-bore tube strapped to tubing, retrievable on wireline but conduit requires workover to replace; (2) FIPA pumped fiber — dual-conduit turnaround loop with turnaround sub at bottom; fiber sensing element hydraulically pumped out and replaced at surface with no rig (truly rigless); conduit loop itself is permanent and requires workover if it fails; informs fiber_optics intervention_type default (full workover for conventional) and FIPA override (rigless for fiber element) |
